...
function authenticate() {
    ...
    if (isset($headers['Authorization'])) {
        ...
        $isValidApiKey = $db->entityManager->afficheur("api_key = ?", $api_key)->fetch();
        //$isValidApiKey = $db->entityManager->fournisseur("api_key = ?", $api_key)->fetch();
    ...
    else
    {
        ...
        global $app;
        echoResponse(400, false, "Vous ne pouvez pas accéder à cette ressource. Clé API absente ou non conforme dans l'en-tête", NULL);
        $app->stop();
    }
}

/**
 * Ajout de Couche intermédiaire pour authentifier chaque demande avec params
 * Vérifier si la demande a clé API valide dans l'en-tête "Authorization"
 */
function authenticateWithParams($tableName) {
    $headers = apache_request_headers(); // Obtenir les en-têtes de requêtes
    ...
    if (isset($headers['Authorization'])) {
        ...
        $isValidApiKey = $db->entityManager->$tableName("api_key = ?", $api_key)->fetch();
        //$isValidApiKey = $db->entityManager->fournisseur("api_key = ?", $api_key)->fetch();
